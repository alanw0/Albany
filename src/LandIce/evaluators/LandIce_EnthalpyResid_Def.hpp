...
//      ScalarT scale = 0.5 - atan(flux_reg_coeff * diffEnth(cell,node))/pi;
      for (std::size_t qp = 0; qp < numQPs; ++qp)
      {
        ...
        //scale = Albany::ADValue(scale);
        Residual(cell,node) += powm3 * (scale * K_i + (1-scale) * nu) * (EnthalpyGrad(cell,qp,0)*wGradBF(cell,node,qp,0) +
...
//                 Albany::ADValue(Velocity(cell,qp,1))*EnthalpyGrad(cell,qp,1) + w*EnthalpyGrad(cell,qp,2))*wBF(cell,node,qp)/scyr ;
        ...
        Residual(cell,node) += powm9*(1 - scale) * k_i * (meltTempGrad(cell,qp,0)*wGradBF(cell,node,qp,0) +
